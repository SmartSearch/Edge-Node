 ...
 *
 * Changes done:
 *  Try to URL encode the dbName in the constructor to support dbNames 
 *    with forward slashes '/'
 *  Added putJsonText(URI uri, String jsonText) function
 *
 */
...
abstract class CouchDbClientBase {
	...
	protected CouchDbClientBase(CouchDbConfig config) {
		...
		String encodedName=config.getDbName();
		try {
			encodedName=URLEncoder.encode(config.getDbName(),"UTF-8");
		} catch (UnsupportedEncodingException e) {
		}

		dbURI   = builder(baseURI).path(encodedName).path("/").build();
	}
     ...
     * @return {@link Response}
     */
    Response putJsonText(URI uri, String jsonText) {
    	assertNotEmpty(jsonText, "jsonText");
        HttpResponse response = null;
        try {
        	String id = Json.getSimpleTextKey(jsonText, "_id"); 
        	if (id==null)
        		id = generateUUID();
            HttpPut put = new HttpPut(builder(uri).path(id).build());
            setEntity(put, jsonText);
            response = executeRequest(put);
            return getResponse(response);
        } finally {
            close(response);
